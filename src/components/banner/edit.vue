        ...
        <el-form ref="form"
          ...
          <el-form-item label="广告链接"
            ...
            <el-input v-model="form.url"
                      placeholder="跳转到商品是输入商品ID即可，打开网页时请输入公众号文章"></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    async _getData () {
      let res = await getData(`banner/read?id=${this.id}`)
      if (res.error_code === 10000) {
        this.form = res.data
      }
    },
    async _create () {
      let data = await getData(`/banner/update/${this.id}`, this.form, 'POST')
