      ...
      <div class="card">
        <div class="card-header">
          <el-breadcrumb>
            ...
            <el-breadcrumb-item>文章分类列表</el-breadcrumb-item>
          </el-breadcrumb>
        ...
        <div class="card-body text">
          <el-table :data="data">
            ...
            <el-table-column prop="image" width="150" label="图片">
  ...
  methods: {
    async _request() {
      let res = await request(
        `/article_category?page=${this.page}&title=${this.title}`
      )
    ...
    create() {
      store.commit('add_tabs', { path: 'add_article_category', name: '添加分类' })
      this.defaultTab = this.$route.path
    },
    async _change(id, key, status) {
      let res = await request(`/article_category/${id}`, { status: status }, 'PUT')
      if (res.error_code === 10000) {
    ...
    async _detele(id, key) {
      let res = await request(`/article_category/${id}`, '', 'DELETE')
      if (res.error_code === 10000) {
    ...
    _edit(id) {
      ...
      store.commit('add_tabs', { path: 'edit_article_category', name: '编辑分类' })
    },
